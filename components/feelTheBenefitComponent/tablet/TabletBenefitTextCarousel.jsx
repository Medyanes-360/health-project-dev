...
const TabletBenefitTextCarousel = () => {
  const settings = {
    dots: false,
    infinite: true,
    speed: 500,
  ...
  return (
    <Slider {...settings}>
      {benefitTexts.map((slide, slideIndex) => (
        <div key={slideIndex} className="p-6">
          {" "}
          {slide.map((benefit, index) => (
            <div key={index} className="mb-8">
              {" "}
              <h3 className="text-[24px] font-semibold text-[#1E1E1E] leading-[32px] mb-4">
                {benefit.title}
              </h3>
              <p className="text-[18px] font-light text-[#1E1E1E] leading-[28px]">
                {benefit.text}
              </p>
            </div>
          ))}
...
export default TabletBenefitTextCarousel;
